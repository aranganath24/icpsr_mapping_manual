...
Now that we've taken care of those preliminary steps, let's bring the tutorial data into our R environment so that we can begin working with it. There are two pieces of data we'll need to load:

* The ICPSR tabular dataset on government policy responses to Covid-19 (see Section 4.1.1)
...
Now, let's go ahead and load the "Dataset" sheet of the ICPSR data file into R Studio. Type the following code into your script, and run it:
...
As we noted above, ```read_excel``` is the function used to bring in Excel spreadsheet data into R. The function has two arguments; the first ("Gov_Responses2Covid19_last.xlsx") is the name of the file we want to import, while the second (sheet="Dataset") specifies that we specifically want to import the the "Dataset" sheet from that Excel file.
...
Note that when printing a dataset to the console, R Studio will only display the first few records (i.e. rows) in the dataset, and won't display all of the fields (i.e. columns) either, but rest assured that all of the information has been imported. 
...
To understand the concept of an object, consider the following simple example

```{r}
x<-5
```

In the code above, we use R's assignment operator, ```<-```, to assign the value 5 to an object named "x." Now that an object named "x" has been created and assigned the value 5, printing "x" in our console (or printing it in our script and running it) will return the value 5: 
...
While the example above was very simple, we can assign virtually any data structure (such as datasets, maps, graphs) to an object in R. Indeed, we'll use the basic principle of object assignment introduced above to assign our ICPSR dataset of interest to an object, which we will call "covid_data" (note that object names are arbitrary and could be virtually anything, but it is good practice for object names to describe the values/data assigned to them):

```{r, echo=-1}
setwd("/Users/adityaranganath/Documents/git_repositories/icpsr_mapping_manual/tutorial_data")
# Imports "Dataset" sheet from ICPSR Excel file into R Studio, and assigns the dataset to a variable called "covid_data"
covid_data<-read_excel("Gov_Responses2Covid19_last.xlsx", sheet="Dataset")
```

The code above imports the data of interest using the code we used above (```read_excel("Gov_Responses2Covid19_last.xlsx", sheet="Dataset")```, but then takes the additional step of assigning the value of this expression (i.e. the imported ICPSR dataset) on the right side of the assignment operator to an object named ```covid_data```. In other words, the code assigns the output of the expression that results from running ```read_excel("Gov_Responses2Covid19_last.xlsx", sheet="Dataset")``` (i.e. the ICPSR dataset) to an object named ```covid_data```. 
...
```{r}
# Prints value of "covid_data" object to the console
covid_data
```

In short, once we've created an object by assigning a value to an object name with the assignment operator (```<-```), we can reference this value by the object's name. For example, in this case, we can refer to the ICPSR Covid-19 data that has been assigned to the ```covid_data``` object as ```covid_data``` within our scripts and documentation. In other words, due to the assignment operation, R is able to recognize that whenever we mention ```covid_data```, we are referring to the underlying value that has been assigned to the ```covid_data``` object (i.e. the ICPSR dataset). Throughout the tutorial, we'll sometimes use object names to refer to the values that they have been assigned; for instance, when we refer to the ```covid_data``` object, we are using it as shorthand for the ICPSR Covid-19 policy responses dataset that has been assigned to this object. 

When we print ```covid_data``` in our console (or run it from our script), R will print only a snapshot of the ```covid_data``` object to the console. However, when doing empirical work, it's usually useful to inspect the entire dataset of interest. We can do so using the ```View``` function. In this case, to inspect the ICPSR dataset that we loaded into R Studio and assigned to ```covid_data```, we can pass ```covid_data``` as an argument to the ```View``` function:

```{r}
# Opens "covid_data" in R Studio's data viewer
View(covid_data)
```

This will open up the dataset within a separate tab in the "Source" window. Within your R Studio environment, the result of running ```View(covid_data)``` will look something like this (dataset outlined in red):
...
The first argument passed to the ```relocate``` function ("country_boundaries") indicates the dataset we want to manipulate using the ```relocate``` function, while the second argument ("geometry") indicates the name of the column in "country_boundaries" that we want to move to the front of the dataset. Once that column relocation has taken place, the ```View``` function brings up the amended dataset as a new tab in the source window. It will look something like this (note the "geometry" column): 
...
tmap_mode("plot")
# maps geographic features (i.e. countries) of "country_boundaries"
tm_shape(country_boundaries)+
  tm_polygons()
```

We can assign this map rendered from the spatial to a distinct object, just as we assigned the tabular ICPSR dataset and the spatial dataset to distinct objects ("covid_data" and "country_boundaries", respectively). Let's call this object "worldmap"; the code below performs this assignment: 
...
Now, let's see this change reflected in the corresponding map. To do so, we must update the "worldmap" object by rerunning the object assignment. That is, "worldmap" will not automatically reflect the edits we just made to the "country_boundaries" dataset. We need to run the code again (with the new version of "country_boundaries" passed through the ```tm_shape``` function) and then assign the revised map to "worldmap", which effectively replaces the previous map (the one with Antarctica):
...
Let's create this smaller dataset, comprised of the ISO code and the economic intervention index of interest, and assign it to an object named "covid_data_economic": 
...
Within the "Source" window, a new tab that contains the smaller 2-column that's been assigned to "covid_data_economic" will appear:
...
Note that the original Covid-19 policy responses dataset, that we previously assigned to the "covid_data" object, is unaffected by our creation and assignment of a new dataset based on the selection of these two variables. We can confirm that this original data ("covid_data") remains unaffect by inspecting it through a call to the ```View``` function:
...
Recall that the data we want to map is contained within the "Economic_Measures" column in "covid_data_economic". R has six fundamental "types" of data: character, numeric, integer, logical, and complex. The code below extracts the "Econmic_Measures" column from the dataset (placing a dollar sign in between the name of the object to which a dataset has been assigned, and a column within that dataset, as in ```covid_data_economic$Economic_Measures```, is a way of isolating and extracting a column from a larger dataset), and then passes it to the ```class``` function to identify the class of the data in the "Economic_Measures" field. 
...
In the code above, the code to the right of the assignment operator extracts  the "Economic_Measures" column from the the "covid_data_economic" dataset (```covid_data_economic$Economic_Measures```), and then changes this column's class to numeric by passing it to the ```as.numeric``` function. The assignment operator then replaces the old version of the "Economic_Measures" column (encoded as character) within "covid_data_economic", with this new (numeric) version of the "Economic_Measures" column. At this point, the "Economic_Measures" column in ```covid_data_economic```  is numeric; we can confirm this by passing the column through the ```class``` function once again:
...
* Finally, we take our newly created dataset, which adds the data we want to map our original spatial dataset (```country_boundaries```), and assign this value to a new object that we can use to build our map(s). We will name this object ```covid_economic_spatial_dataset``` 
...
After moving these variables by using the ```relocate``` function with the relevant variables as arguments, pass ```worldmap_covid_data_economic```, whose variables have now been relocated, to the familiar ```View``` function:
...
Note that we now have a spatial dataset of country boundaries (as indicated by the "name" and "geometry" fields), with the data we want to map (the mean, for each country, of the Covid-19 Economic intervention index during the period under consideration) now included as a column within the spatial dataset. 
...
Recall that as with with anything else in R, we can assign maps created with the *tmap* package to objects, which makes it easy to subsquently reference our maps and customize them as needed. Below, we'll create a rough draft of a map and assign it to an object named ```covid_economic_map```:  
...
The ```tm_polygons``` function can take a variety of arguments that can help us to customize our map; we'll introduce a few more arguments below, but recall that if we want to see the full range of possible arguments, they are presented in the function's documentation (```?tm_polygons```). Now, let's consider the arguments that were passed to the ```tm_polygons``` function in the code above:
...
To see what the map looks like, simply print ```covid_economic_map``` and run it in your script:

```{r, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
covid_economic_map
```
...
```{r a, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
covid_economic_map_alternative
```

Note that we can easily remove the map frame if we'd prefer not to have that, by using the "Frame" argument within the ```tm_layout``` function: 

```{r b, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
...
The biggest issue with the rough drafts of our map from the previous section is the position of the legend. Let's go back to the first map we made (```covid_economic_map```), and simply switch its legend position, by specifying ```position=c("left","bottom")``` withing the ```tm_legend``` function: 

```{r c, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
...
Currently, our legend is discrete, but we may want to try using a continuous legend to better match the continuous nature of the index. We can do this by setting ```style="cont``` within the ```tm_polygons``` function:

```{r d, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
...
```{r e, fig.asp=0.5, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
covid_economic_map<- tm_shape(covid_economic_spatial_dataset)+
...
* The ```tm_layout``` function also takes many arguments:
  ...
  * ```legend.outside=TRUE``` specifies the legend is to be situated outside the (invisible) map frame; this prevents the legend from blocking out portions of the map itself
...
The best way to get a sense of how all of these various arguments work is to try changing the values of the arguments and then inspect their effect on the map's appearance. 

### Add Map Title and Credits

One useful thing to keep in mind is that when adding new elements to our map, it's not necessary to retype all of our code; thus far, that is what we've been doing, but it's also possible to simply append changes to an object. Below, to the right of the assignment operator, we first print the name of the object we want to modify (i.e. ```covid_economic_map```) and then type a "+" sign; then, we can modify the object as desired through the use of relevant functions, and then save those changes by assigning the map back to an object named ```covid_economic_map```. In particular, we want to add and customize a map title (which we will do by passing a few additional arguments to the now-familiar ```tm_layout``` function), as well as add and customize a "Map Credits" section, which will include some basic map metadata. 

```{r f, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
...
It's worth emphasizing that when working on your own maps, you expect to engage in a lot of trial and error to customize the map's appearance to your specifications, and to determine the appropriate values for the various function arguments. In addition, there are a lot of *tmap* functions and arguments that we haven't covered; be sure to explore all the possibilities  by exploring *tmap's* documentation (```?tmap```)
...
```{r g, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
...
# Saves "covid_economic_map" object as png file to working directory with resolution of 1920 by 1080 pixels
tmap_save(covid_economic_map, "covid_economic_map_export.png", width=1920, height=1080)
```

JPEG File
```{r, warning=FALSE, message=FALSE, eval=FALSE}
# Saves "covid_economic_map" object as jpeg file to working directory with resolution of 1920 by 1080 pixels
tmap_save(covid_economic_map, "covid_map.jpeg", width=1920, height=1080)
```

PDF File
```{r, warning=FALSE, message=FALSE, eval=FALSE}
# Saves "covid_economic_map" object as pdf file to working directory with resolution of 1920 by 1080 pixels
tmap_save(covid_economic_map, "covid_map.pdf", width=1920, height=1080)
```

Once we've exported the map in the format we'd like, we can open up the file from our working directory. The PNG file will look something like this: 

```{r, echo=FALSE, results='asis', out.width='100%', fig.cap='Map Exported as Image File'}
knitr::include_graphics('images/pngscreenshot.png')
```
...
```{r, echo=FALSE, results='asis', out.width='100%', fig.cap='Export Map Using R Studio GUI'}
knitr::include_graphics('images/exportalt.png')
```

## Make a Dynamic Map 

So far, we've been making static print maps, which are a powerful tool of visualization; however, it's also possible to make "dynamic" (or interactive) maps where you can pan across the map and zoom into areas of interest. Dynamic maps can therefore be a very useful tool for exploratory data analysis. In addition, *tmap* produces these dynamic maps as html output, which means we can embed these maps within web pages, if a project involves a web-hosting component. 

To build such a dynamic map, simply  build a map in tmap (just as we would build a static map) and assign it to an object. Then, use the ```tmap_mode``` function to set *tmap* to ```view``` mode. At this point, if you display  the map object by printing its name, *tmap* will generate a dynamic map.

```{r, message=FALSE, warning=FALSE}

# Make map and assign it to an object named "covid_economic_webmap" (Same procedure as for static map)
covid_economic_webmap<-tm_shape(covid_economic_spatial_dataset)+
                        tm_polygons(col="mean_economic", n=5, style="jenks", palette="BuGn")
# Set tmap mode to "view
tmap_mode("view")

# display "covid_economic_webmap" in view mode
covid_economic_webmap
```
\
Within our R Studio interface, the dynamic map appears in the "Viewer" tab on the bottom-left window. To open the interactive map up in a larger window, click "Zoom". Note that when we click on a country, the mean value of the index will pop out; we can also pan around the map and zoom into areas of interest by clicking the large "+" button:


```{r, echo=FALSE, results='asis', out.width='100%', fig.cap='Dynamic Map Within R Studio Interace'}
knitr::include_graphics('images/dynamic.png')
```

It's possible to customize dynamic maps to some degree, but not to the same extent as static maps. Consult *tmap's* documentation for additional guidance. 

It's possible to export dynamic maps as html files. To do so, we use the same ```tmap_save``` function as before, but instead specify an html extension. The html file will be written to your working directory:

```{r, message=FALSE, eval=FALSE}
# Writes dynamic map to disk as html file
tmap_save(covid_economic_webmap, "covid_economic_webmap.html")
```

Once it's been saved, we can open up the interactive map in a web browser of choice:

```{r, echo=FALSE, results='asis', out.width='100%', fig.cap='Saved Interactive Map Open in Browser'}
knitr::include_graphics('images/webmap_browser.png')
```

We can switch back to "plot" mode, at which point *tmap* will go back to making static maps.  

```{r, message=FALSE, warning=FALSE }
tmap_mode("plot")
```

We can toggle back and forth between "plot" mode and "view" mode as necessary, depending on whether we want to make a static or dynamic map.

## Summary 

After working through the tutorial, it can be useful to see the essential steps required to generate our final print (i.e. static) map in a single script; note that the script below presupposes that we are in the correct working directory (i.e. the working directory which contains the ICPSR data we downloaded as an Excel file), and that all the libraries are loaded. 
...
Our discussion of dynamic webmaps was of course considerably shorter than our discussion of print maps in R, so the corresponding script is naturally  shorter. The following script goes from reading in the ICPSR dataset to producing a dynamic webmap of the country-level average of the Covid-19 economic intervention index. Note that many of the steps involved are exactly the same as the process of making the print map:
...
# Makes map and assigns it to an object called "covid_economic_webmap"
covid_economic_webmap<-tm_shape(covid_economic_spatial_dataset)+
                        tm_polygons(col="mean_economic", n=5, style="jenks", palette="BuGn")
...
# displays covid_economic_webmap in view mode
covid_economic_webmap
