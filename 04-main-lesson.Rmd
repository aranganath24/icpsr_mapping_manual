...
knitr::include_graphics('images/guiexport.png')
```

## Make a Dynamic Map 

So far, we've been making static print maps, which are a powerful tool of visualization; however, it's also possible to make "dynamic" maps where you can pan across the map and zoom into areas of interest. Dynamic maps can therefore be a very useful tool for exploratory data analysis. In addition, *tmap* produces these dynamic maps as html output, which means we can embed these maps within web pages, in case that would be preferable to a static map as a mode of presentation.

To build such a dynamic map, simply  build a map just as we would build a static map, and assign it to an object. Then, use the ```tmap_mode``` function to "view". At this point, display up the map object by printing its name will produce a dynamic map.
