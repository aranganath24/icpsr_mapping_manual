...
```{r, echo=-1, message=FALSE}
tmap_mode("plot")
covid_economic_map_alternative<-tm_shape(worldmap_covid_data_economic)+
...
covid_economic_map_alternative<-covid_economic_map_alternative+
...
* ```tm_layout```
  ...
  * ```inner.margins=c(0.06,0.10,0.10,0.08)``` establishes the size of the margins within the map frame. It's most noticeable impact in this context is to regulate the whitespace between the title and the map features. 
...
It's worth emphasizing that when working on your own maps, expect to engage in a lot of trial and error to customize the map's appearance to your specifications, and to determine the appropriate values for the various function arguments. In addition, there are a lot of *tmap* functions and arguments that we haven't covered; be sure to explore all the possibilities  by exploring *tmap's* documentation (```?tmap```)

### Final Map Code

For convenience, here's all of the code we used to construct the map we arrived at in the previous section, along with comments summarizing what each line is doing:

```{r, message=FALSE, warning=FALSE, echo=-1}
tmap_mode("plot")
covid_economic_map<-  
tm_shape(worldmap_covid_data_economic)+ # Specifies Spatial Dataset
...
The first code block below writes our map object as a png file (with dimensions of 1920 x 1080 pixels) to our working directory; the subsequent two blocks write the map object as jpeg and PDF files, respectively. 
...
So far, we've been making static print maps, which are a powerful tool of visualization; however, it's also possible to make "dynamic" maps where you can pan across the map and zoom into areas of interest. Dynamic maps can therefore be a very useful tool for exploratory data analysis. In addition, *tmap* produces these dynamic maps as html output, which means we can embed these maps within web pages, if a project involves a web-hosting component. 
...
# Make map and assign it to an object (Same procedure as for static map)
covid_economic_webmap<-tm_shape(worldmap_covid_data_economic)+
...
```
\
Within our R Studio interface, the dynamic map appears in the "Viewer" tab on the bottom-left window. To open up a larger window, click "Zoom". Note that when we click on a country, the mean value of the index will pop out; we can also pan around the map and zoom into areas of interest by clicking the large "+" button:
...
It's possible to export dynamic maps as html files. To do so, we use the same ```tmap_save``` function as before, but simply specify an html extension. The html file will be written to your working directory:
...
# Selects "iso" and "Economic_Measures" variables from ICPSR Dataset, and assigns this new dataset to "covid_data_economic" object
covid_data_economic<-covid_data %>% 
                      select(iso, Economic_Measures) 
...
# Join dataset with country-level means for "Economic_Measures" index (in "covid_data_economic" object) to spatial dataset of world boundaries (in "country_boundaries" object), based on common 3-Digit ISO Codes

worldmap_covid_data_economic<-full_join(country_boundaries, covid_data_economic_avg, by=c("iso_a3"="iso"))

# Make static map and assigns to object called "covid_economic_map"
