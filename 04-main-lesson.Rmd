...
We can assign this map (i.e. a visual representation of the "country_boundaries" dataset) to an object, just as we assigned the ICPSR tabular dataset to an object. Let's call this object "worldmap."
...
Having assigned the map to the "worldmap" object, we can bring up the map whenever we want by simply calling the object by typing "worldmap" into the script and running it:

```{r, fig.asp=0.5}
# calls "worldmap" object to display country-level map generated from the "country_boundaries" spatial dataset's geographic features
worldmap
```
...
```{r, echo=FALSE, results='asis', out.width='100%', fig.cap='Map in "Plots" Tab'}
...
We can view the attribute table that corresponds to the features displayed above by simply passing the name of the object containing the spatial dataset (i.e. "country_boundaries") to the ```View``` function:

```{r}
# View attribute table of "country_boundaries" spatial dataset
View(country_boundaries)
```

```{r, echo=FALSE}
kable(head(country_boundaries))
```

In your R Studio interface, this attribute table should appear as a tab in your "Source" window, in the same location as the ICPSR Covid-19 dataset earlier. We can go ahead and scroll across and scroll up/down the attribute table to explore the information it contains. Remember that each row in the table corresponds to a country in the feature layer, which we just learned how to visualize. Every column in the table corresponds to a different attribute, or piece of country-level information. 
...
To delete Antarctica from the map, we first need to delete the row that corresponds to Antarctica from the spatial dataset contained in the "country_boundaries" dataset. We can do so with the following code:
...
Let's unpack what the code above is doing. First, we have to decide whether we want to assign the edited spatial dataset to a new object, or simply update the existing "country_boundaries" object and overwrite the current version of the object. The first option is akin to doing a "save as" and creating a new document with relevant changes in word processing software, while the latter is akin to doing a "save" and appending those changes to the existing file. To keep things easy, we'll simply overwrite the current "country_boundaries" object with the changes, rather than making a new object; as a result, the name of the object to the left of the assignment operator (<-) remains "country_boundaries."

To the right of the assignment operator, we need to tell R what to assign to the amended "country_boundaries" object. We want to assign the existing country_boundaries object, sans Alaska, to the amended object. Therefore, we start by once again typing "country_boundaries" to indicate that the dataset (with Antarctica) in this existing object is our starting point. With this starting point established, we then need to indicate how we want to alter the existing object; we do this to the right of the "pipe" operator (%>%), which is used to take the output of the code on its left, and then use that output as an input to the code on its right. Here, the pipe is taking the output of the "country_dataset" object (i.e. the existing spatial dataset) on its left, and then using that output as an input to the ```filter``` function on its right. In other words, the pipe operator links the code on its two sides, and indicates that the object being passed through the ```filter``` function is the "country_boundaries" object. 

The ```filter``` function  is a function from the *dplyr* package that allows one to select rows from a dataset by specified criteria. In our case, we want to select all rows from the dataset that are not Antarctica. Antarctica's 3 digit ISO code is ATA (this information is in the spatial dataset's attribute table), and we'll use this ISO code to specify our desired criteria. The code that reads ```filter(iso_a3!="ATA")``` is essentially our specification; it is effectively saying "select all rows in the dataset that do not have a 3 digit ISO code of ATA (which corresponds to Antarctica)." The exclamation mark followed by the equals sign (!=) is R syntax for "not equals." If we were to instead type ```filter(iso_a3=="ATA)```, the function would *only* select the Antarctica row from the dataset and discard everything else. 

There was a lot to unpack in that single line of code, but ultimately it is fairly intuitive. We can translate that code into ordinary language as follows: "Take the existing country boundaries dataset and then select only the countries that are not Antarctica (as indicated by the fact that their 3-digit ISO codes are NOT "ATA"). Then, take this new dataset, which doesn't include Antarctica, and assign it back to the existing 'country_boundaries' object, which effectively replaces the previous spatial dataset in the country_boundaries object (which did include Antarctica)."
